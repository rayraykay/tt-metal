...
AddressableCoreType get_core_type(uint8_t noc_id, uint8_t x, uint8_t y, bool& is_virtual_coord) {
    ...
    for (uint32_t idx = 0; idx < MAX_PHYSICAL_NON_WORKER_CORES; idx++) {
        ...
        if (x == NOC_0_X_PHYS_COORD(noc_id, core_info->noc_size_x, (uint32_t)core_x) &&
            ...
            // DPRINT << "noc_size " << (uint32_t)core_info->noc_size_x << " " << (uint32_t)core_info->noc_size_y << "
            // noc id " << (uint32_t)noc_id << ENDL(); DPRINT << "here 0: " << (uint32_t)NOC_0_X_PHYS_COORD(noc_id,
            // core_info->noc_size_x, (uint32_t)core_x)
            //        << ", " << (uint32_t)NOC_0_Y_PHYS_COORD(noc_id, core_info->noc_size_y, (uint32_t)core_y) <<
            //        ENDL();
...
#endif
        // DPRINT << "sani x " << (uint32_t)x << " y " << (uint32_t)y
        //        << " x start " << (uint32_t)NOC_0_X(noc_id, core_info->noc_size_x, (uint32_t)VIRTUAL_TENSIX_START_X)
        //        << " x end " << (uint32_t)NOC_0_X(noc_id, core_info->noc_size_x, (uint32_t)virtual_end_x)
        //        << " y start " << (uint32_t)NOC_0_X(noc_id, core_info->noc_size_y, (uint32_t)VIRTUAL_TENSIX_START_Y)
        //        << " y end " << (uint32_t)NOC_0_X(noc_id, core_info->noc_size_y, virtual_end_y) << ENDL();
...
uint32_t debug_sanitize_noc_addr(
    ...
    DPRINT << "first x " << (uint32_t)x << " y " << (uint32_t)y << ENDL();
    AddressableCoreType core_type = get_core_type(noc_id, x, y, is_virtual_coord);
    DPRINT << "is virtual " << (uint32_t)is_virtual_coord << ENDL();
    ...
    if (multicast) {
        ...
        // DPRINT << "x " << (uint32_t)x << " x end " << (uint32_t)x_end << " y " << (uint32_t)y << " y end " <<
        // (uint32_t)y_end << ENDL(); DPRINT << "virtual s " << (uint32_t)is_virtual_coord << " virtual end " <<
        // (uint32_t)is_virtual_coord_end << ENDL(); Multicast supports workers only
        ...
        if (is_virtual_coord && is_virtual_coord_end) {
            ...
            // DPRINT << "x " << x << " x end " << x_end << " y " << y << " y end " << y_end << ENDL();
            if (noc_id == 0) {
                if (x > x_end || y > y_end) {
