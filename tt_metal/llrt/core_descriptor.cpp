...
const core_descriptor_t& get_core_descriptor_config(
    ...
    ARCH arch = tt::tt_metal::MetalContext::get_cluster().arch();
    uint32_t harvesting_mask = tt::tt_metal::MetalContext::get_cluster().get_harvesting_mask(device_id);
    ...
    if (tt::tt_metal::MetalContext::get_cluster().is_galaxy_cluster()) {
        if (tt::tt_metal::MetalContext::get_cluster().get_board_type(device_id) == BoardType::N150) {
            ...
            TT_ASSERT(
                tt::tt_metal::MetalContext::get_cluster().get_board_type(device_id) == BoardType::GALAXY,
    ...
    if (tt::tt_metal::MetalContext::get_cluster().is_galaxy_cluster() and product_name == "nebula_x1") {
    ...
    auto logical_active_eth_cores = tt::tt_metal::MetalContext::get_cluster().get_active_ethernet_cores(device_id);
