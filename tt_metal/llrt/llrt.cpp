...
std::vector<uint32_t> read_hex_vec_from_core(chip_id_t chip, const CoreCoord &core, uint64_t addr, uint32_t sz_bytes) {
    ...
    tt::tt_metal::MetalContext::get_cluster().read_core(read_hex_vec, sz_bytes, tt_cxy_pair(chip, core), addr);
    return read_hex_vec;
}

CoreCoord logical_core_from_ethernet_core(chip_id_t chip_id, const CoreCoord &ethernet_core) {
    return tt::tt_metal::MetalContext::get_cluster().get_logical_ethernet_core_from_virtual(chip_id, ethernet_core);
}
...
void print_worker_cores(chip_id_t chip_id) {
    ...
    for (const CoreCoord& core : tt::tt_metal::MetalContext::get_cluster().get_soc_desc(chip_id).get_cores(
...
bool test_load_write_read_risc_binary(
    ...
    assert(
        tt::tt_metal::MetalContext::get_cluster().is_worker_core(core, chip_id) or
        tt::tt_metal::MetalContext::get_cluster().is_ethernet_core(core, chip_id));
    ...
    if (std::getenv("TT_METAL_KERNEL_READBACK_ENABLE") != nullptr) {
        tt::tt_metal::MetalContext::get_cluster().l1_barrier(chip_id);
...
CoreCoord get_core_for_dram_channel(int dram_channel_id, chip_id_t chip_id) {
    return tt::tt_metal::MetalContext::get_cluster().get_soc_desc(chip_id).get_preferred_worker_core_for_dram_view(
        dram_channel_id);
}
...
static bool check_if_riscs_on_specified_core_done(chip_id_t chip_id, const CoreCoord &core, int run_state) {
    bool is_eth_core = tt::tt_metal::MetalContext::get_cluster().is_ethernet_core(core, chip_id);
    ...
    if (is_eth_core) {
        auto active_eth_cores = tt::tt_metal::MetalContext::get_cluster().get_active_ethernet_cores(chip_id);
        auto inactive_eth_cores = tt::tt_metal::MetalContext::get_cluster().get_inactive_ethernet_cores(chip_id);
