...
bool WriteToDeviceDRAMChannel(IDevice* device, int dram_channel, uint32_t address, std::vector<uint32_t>& host_buffer) {
    ...
    tt::tt_metal::MetalContext::get_cluster().write_dram_vec(
...
bool ReadFromDeviceDRAMChannel(
    ...
    bool pass = true;
    tt::tt_metal::MetalContext::get_cluster().dram_barrier(device->id());
    tt::tt_metal::MetalContext::get_cluster().read_dram_vec(
...
bool WriteRegToDevice(IDevice* device, const CoreCoord& logical_core, uint32_t address, const uint32_t& regval) {
    ...
    tt::tt_metal::MetalContext::get_cluster().write_reg(&regval, tt_cxy_pair(device->id(), worker_core), address);
    return true;
}
...
bool ReadRegFromDevice(IDevice* device, const CoreCoord& logical_core, uint32_t address, uint32_t& regval) {
    tt::tt_metal::MetalContext::get_cluster().l1_barrier(device->id());
    auto worker_core = device->worker_core_from_logical_core(logical_core);
    tt::tt_metal::MetalContext::get_cluster().read_reg(&regval, tt_cxy_pair(device->id(), worker_core), address);
    return true;
}

void InitializeFabricConfig(FabricConfig fabric_config) {
    tt::tt_metal::MetalContext::get_cluster().initialize_fabric_config(fabric_config);
}
...
std::map<chip_id_t, IDevice*> CreateDevices(
    ...
    ZoneScoped;
    bool is_galaxy = tt::tt_metal::MetalContext::get_cluster().is_galaxy_cluster();
...
void LaunchProgram(IDevice* device, Program& program, bool wait_until_cores_done) {
    {  // Profiler scope start
        ...
        tt::tt_metal::MetalContext::get_cluster().dram_barrier(device_id);
...
size_t GetNumAvailableDevices() { return tt::tt_metal::MetalContext::get_cluster().number_of_user_devices(); }

bool IsGalaxyCluster() { return tt::tt_metal::MetalContext::get_cluster().is_galaxy_cluster(); }

size_t GetNumPCIeDevices() { return tt::tt_metal::MetalContext::get_cluster().number_of_pci_devices(); }

chip_id_t GetPCIeDeviceID(chip_id_t device_id) {
    return tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(device_id);
}
