...
void append_fabric_connection_rt_args(
    ...
    const auto& neighbor_chips_and_cores =
        tt::tt_metal::MetalContext::get_cluster().get_ethernet_cores_grouped_by_connected_chips(src_chip_id);
    ...
    const auto& fabric_ethernet_channels =
        tt::tt_metal::MetalContext::get_cluster().get_fabric_ethernet_channels(src_chip_id);
    const auto& candidate_ethernet_cores = dst_chip_and_cores->second;
    const auto& logical_eth_core_to_chan_map =
        tt::tt_metal::MetalContext::get_cluster().get_soc_desc(src_chip_id).logical_eth_core_to_chan_map;
    ...
    CoreCoord fabric_router_virtual_core = tt::tt_metal::MetalContext::get_cluster().get_virtual_eth_core_from_channel(
