...
static CoreDescriptorSet GetAllCores(chip_id_t device_id) {
    ...
    for (const auto& logical_core : tt::tt_metal::MetalContext::get_cluster().get_active_ethernet_cores(device_id)) {
        all_cores.insert({logical_core, CoreType::ETH});
    }
    for (const auto& logical_core : tt::tt_metal::MetalContext::get_cluster().get_inactive_ethernet_cores(device_id)) {
        all_cores.insert({logical_core, CoreType::ETH});
    }
...
static CoreDescriptorSet GetDispatchCores(chip_id_t device_id) {
    ...
    unsigned num_cqs = tt::tt_metal::MetalContext::get_dispatch_core_manager().get_num_hw_cqs();
...
static tt::tt_metal::HalProgrammableCoreType get_programmable_core_type(CoreCoord virtual_core, chip_id_t device_id) {
    if (!tt::tt_metal::MetalContext::get_cluster().is_ethernet_core(virtual_core, device_id)) {
        return tt::tt_metal::HalProgrammableCoreType::TENSIX;
    }

    // Eth pcores have a different address, but only active ones.
    CoreCoord logical_core =
        tt::tt_metal::MetalContext::get_cluster().get_logical_ethernet_core_from_virtual(device_id, virtual_core);
    auto active_ethernet_cores = tt::tt_metal::MetalContext::get_cluster().get_active_ethernet_cores(device_id);
...
inline int GetNumRiscs(const CoreDescriptor& core) {
    if (core.type == CoreType::ETH) {
        return (tt::tt_metal::MetalContext::get_cluster().arch() == tt::ARCH::BLACKHOLE) ? DPRINT_NRISCVS_ETH + 1
                                                                                         : DPRINT_NRISCVS_ETH;
    } else {
