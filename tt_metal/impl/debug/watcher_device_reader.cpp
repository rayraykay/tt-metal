...
static CoreType core_type_from_virtual_core(chip_id_t device_id, const CoreCoord& virtual_coord) {
    if (tt::tt_metal::MetalContext::get_cluster().is_worker_core(virtual_coord, device_id)) {
        return CoreType::WORKER;
    } else if (tt::tt_metal::MetalContext::get_cluster().is_ethernet_core(virtual_coord, device_id)) {
        return CoreType::ETH;
    }

    const metal_SocDescriptor& soc_desc = tt::tt_metal::MetalContext::get_cluster().get_soc_desc(device_id);
    CoreType core_type =
...
static CoreCoord virtual_noc_coordinate(chip_id_t device_id, uint8_t noc_index, CoreCoord coord) {
    auto grid_size = tt::tt_metal::MetalContext::get_cluster().get_soc_desc(device_id).grid_size;
...
WatcherDeviceReader::WatcherDeviceReader(
    ...
    if (tt::tt_metal::MetalContext::get_cluster().arch() == ARCH::WORMHOLE_B0 &&
        tt::llrt::RunTimeOptions::get_instance().get_watcher_enabled()) {
        std::vector<uint32_t> read_data;
        for (const CoreCoord& eth_core :
             tt::tt_metal::MetalContext::get_cluster().get_active_ethernet_cores(device_id)) {
            CoreCoord virtual_core =
                tt::tt_metal::MetalContext::get_cluster().get_virtual_coordinate_from_logical_coordinates(
...
void WatcherDeviceReader::Dump(FILE* file) {
    ...
    uint8_t num_hw_cqs = tt::tt_metal::MetalContext::get_dispatch_core_manager().get_num_hw_cqs();
    DispatchCoreConfig dispatch_core_config =
        tt::tt_metal::MetalContext::get_dispatch_core_manager().get_dispatch_core_config();
    ...
    // Dump worker cores
    CoreCoord grid_size =
        tt::tt_metal::MetalContext::get_cluster().get_soc_desc(device_id).get_grid_size(CoreType::TENSIX);
    ...
    // Dump eth cores
    for (const CoreCoord& eth_core : tt::tt_metal::MetalContext::get_cluster().get_active_ethernet_cores(device_id)) {
        CoreDescriptor logical_core = {eth_core, CoreType::ETH};
        DumpCore(logical_core, true);
    }
    for (const CoreCoord& eth_core : tt::tt_metal::MetalContext::get_cluster().get_inactive_ethernet_cores(device_id)) {
        CoreDescriptor logical_core = {eth_core, CoreType::ETH};
        DumpCore(logical_core, false);
    }
...
void WatcherDeviceReader::DumpCore(CoreDescriptor& logical_core, bool is_active_eth_core) {
    ...
    virtual_core.coord = tt::tt_metal::MetalContext::get_cluster().get_virtual_coordinate_from_logical_coordinates(
    ...
    if (tt::llrt::RunTimeOptions::get_instance().get_watcher_phys_coords()) {
        CoreCoord phys_core =
            tt::tt_metal::MetalContext::get_cluster().get_physical_coordinate_from_logical_coordinates(
    ...
    if (is_eth_core) {
        ...
        if (tt::tt_metal::MetalContext::get_cluster().arch() == ARCH::BLACKHOLE) {
...
void WatcherDeviceReader::DumpLaunchMessage(CoreDescriptor& core, const mailboxes_t* mbox_data) {
    ...
    if (!is_eth) {
        ...
        fprintf(f, " ");
    } else if (tt::tt_metal::MetalContext::get_cluster().arch() == ARCH::BLACKHOLE) {
        fprintf(f, "smsg:");
        DumpRunState(core, launch_msg, slave_sync->dm1);
        fprintf(f, " ");
    }
