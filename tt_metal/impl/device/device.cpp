...
std::unordered_set<CoreCoord> Device::get_active_ethernet_cores(bool skip_reserved_tunnel_cores) const {
    return tt::tt_metal::MetalContext::get_cluster().get_active_ethernet_cores(this->id_, skip_reserved_tunnel_cores);
}
...
std::unordered_set<CoreCoord> Device::get_inactive_ethernet_cores() const {
    return tt::tt_metal::MetalContext::get_cluster().get_inactive_ethernet_cores(this->id_);
}

bool Device::is_inactive_ethernet_core(CoreCoord logical_core) const {
    auto inactive_ethernet_cores = tt::tt_metal::MetalContext::get_cluster().get_inactive_ethernet_cores(this->id_);
...
std::tuple<chip_id_t, CoreCoord> Device::get_connected_ethernet_core(CoreCoord eth_core) const {
    return tt::tt_metal::MetalContext::get_cluster().get_connected_ethernet_core(std::make_tuple(this->id_, eth_core));
}

std::vector<CoreCoord> Device::get_ethernet_sockets(chip_id_t connected_chip_id) const {
    return tt::tt_metal::MetalContext::get_cluster().get_ethernet_sockets(this->id_, connected_chip_id);
}

bool Device::is_mmio_capable() const {
    return tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(this->id_) == this->id_;
}
...
void Device::get_associated_dispatch_virtual_cores(
    ...
    if (this->is_mmio_capable()) {
        for (const chip_id_t& device_id :
             tt::tt_metal::MetalContext::get_cluster().get_devices_controlled_by_mmio_device(this->id_)) {
            uint8_t num_hw_cqs = this->num_hw_cqs();
            uint16_t curr_channel =
                tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(device_id);
            CoreType dispatch_core_type = MetalContext::get_dispatch_core_manager().get_dispatch_core_type();
            for (uint8_t cq_id = 0; cq_id < num_hw_cqs; cq_id++) {
                if (device_id == this->id_) {
                    //mmio device.
                    bool dispatch_hd_allocated = false;
                    CoreCoord virtual_core_dispatch_hd;
                    if (MetalContext::get_dispatch_core_manager().is_dispatcher_core_allocated(
                            device_id, curr_channel, cq_id)) {
                        tt_cxy_pair dispatch_location =
                            MetalContext::get_dispatch_core_manager().dispatcher_core(device_id, curr_channel, cq_id);
                    ...
                    if (MetalContext::get_dispatch_core_manager().is_dispatcher_s_core_allocated(
                            device_id, curr_channel, cq_id)) {
                        tt_cxy_pair dispatch_s_location =
                            MetalContext::get_dispatch_core_manager().dispatcher_s_core(device_id, curr_channel, cq_id);
                    ...
                    if (MetalContext::get_dispatch_core_manager().is_prefetcher_core_allocated(
                            device_id, curr_channel, cq_id)) {
                        tt_cxy_pair prefetch_location =
                            MetalContext::get_dispatch_core_manager().prefetcher_core(device_id, curr_channel, cq_id);
                    ...
                    if (MetalContext::get_dispatch_core_manager().is_mux_core_allocated(
                            device_id, curr_channel, cq_id)) {
                        tt_cxy_pair mux_location =
                            MetalContext::get_dispatch_core_manager().mux_core(device_id, curr_channel, cq_id);
                    ...
                    if (MetalContext::get_dispatch_core_manager().is_demux_core_allocated(
                            device_id, curr_channel, cq_id)) {
                        tt_cxy_pair demux_location =
                            MetalContext::get_dispatch_core_manager().demux_core(device_id, curr_channel, cq_id);
        ...
        uint16_t curr_channel = tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(device_id);
        CoreType dispatch_core_type = MetalContext::get_dispatch_core_manager().get_dispatch_core_type();
        for (uint8_t cq_id = 0; cq_id < num_hw_cqs; cq_id++) {
            if (MetalContext::get_dispatch_core_manager().is_dispatcher_core_allocated(
                    device_id, curr_channel, cq_id)) {
            ...
            if (MetalContext::get_dispatch_core_manager().is_mux_core_allocated(device_id, curr_channel, cq_id)) {
                tt_cxy_pair mux_location =
                    MetalContext::get_dispatch_core_manager().mux_core(device_id, curr_channel, cq_id);
                CoreCoord virtual_core = this->virtual_core_from_logical_core(mux_location, dispatch_core_type);
            ...
            if (MetalContext::get_dispatch_core_manager().is_demux_core_allocated(device_id, curr_channel, cq_id)) {
                tt_cxy_pair demux_location =
                    MetalContext::get_dispatch_core_manager().demux_core(device_id, curr_channel, cq_id);
                CoreCoord virtual_core = this->virtual_core_from_logical_core(demux_location, dispatch_core_type);
        ...
        tt_cxy_pair mux_location = MetalContext::get_dispatch_core_manager().mux_d_core(device_id, curr_channel, 0);
        virtual_core = this->virtual_core_from_logical_core(mux_location, dispatch_core_type);
        my_dispatch_cores[mux_location.chip].insert(virtual_core);
        tt_cxy_pair demux_location = MetalContext::get_dispatch_core_manager().demux_d_core(device_id, curr_channel, 0);
        virtual_core = this->virtual_core_from_logical_core(demux_location, dispatch_core_type);
        my_dispatch_cores[demux_location.chip].insert(virtual_core);
        for (uint8_t cq_id = 0; cq_id < num_hw_cqs; cq_id++) {
            tt_cxy_pair prefetch_location =
                MetalContext::get_dispatch_core_manager().prefetcher_d_core(device_id, curr_channel, cq_id);
            virtual_core = this->virtual_core_from_logical_core(prefetch_location, dispatch_core_type);
            my_dispatch_cores[prefetch_location.chip].insert(virtual_core);
        }
        for (uint8_t cq_id = 0; cq_id < num_hw_cqs; cq_id++) {
            tt_cxy_pair dispatch_location =
                MetalContext::get_dispatch_core_manager().dispatcher_d_core(device_id, curr_channel, cq_id);
...
void Device::initialize_cluster() {
    ...
    int ai_clk = tt::tt_metal::MetalContext::get_cluster().get_device_aiclk(this->id_);
...
std::unique_ptr<Allocator> Device::initialize_allocator(size_t l1_small_size, size_t trace_region_size, tt::stl::Span<const std::uint32_t> l1_bank_remap) {
    ZoneScoped;
    const metal_SocDescriptor& soc_desc = tt::tt_metal::MetalContext::get_cluster().get_soc_desc(this->id_);
    const auto& dispatch_core_config = MetalContext::get_dispatch_core_manager().get_dispatch_core_config();
    ...
    AllocatorConfig config(
        {.num_dram_channels = static_cast<size_t>(soc_desc.get_num_dram_views()),
         ...
         .worker_log_to_virtual_routing_x =
             tt::tt_metal::MetalContext::get_cluster().get_worker_logical_to_virtual_x(this->id()),
         .worker_log_to_virtual_routing_y =
             tt::tt_metal::MetalContext::get_cluster().get_worker_logical_to_virtual_y(this->id()),
...
void Device::initialize_device_bank_to_noc_tables(const HalProgrammableCoreType &core_type, CoreCoord virtual_core)
{
    ...
    tt::tt_metal::MetalContext::get_cluster().write_core(
    ...
    switch (core_type) {
        case HalProgrammableCoreType::TENSIX: {
            ...
            if (this->using_slow_dispatch()) {
                ...
                if (MetalContext::get_dispatch_core_manager().get_dispatch_core_type() == CoreType::WORKER) {
                    physical_dispatch_cores = this->worker_cores_from_logical_cores(
                        MetalContext::get_dispatch_core_manager().get_all_logical_dispatch_cores(this->id()));
                }
        ...
        case HalProgrammableCoreType::IDLE_ETH: {
            ...
            if (is_idle_eth or !hal_ref.get_eth_fw_is_cooperative()) {
                tt::tt_metal::MetalContext::get_cluster().assert_risc_reset_at_core(
...
void Device::reset_cores() {
    ...
    auto erisc_app_still_running = [this](CoreCoord virtual_core) {
        ...
        TT_ASSERT(
            (this->arch() == ARCH::WORMHOLE_B0) and
                (tt::tt_metal::MetalContext::get_cluster().is_ethernet_core(virtual_core, this->id())),
    ...
    auto mmio_device_id = tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(this->id_);
    ...
    for (auto &id_and_cores : dispatch_cores) {
        for (auto it = id_and_cores.second.begin(); it != id_and_cores.second.end(); it++) {
            ...
            if (tt::tt_metal::MetalContext::get_cluster().is_ethernet_core(virtual_core, id_and_cores.first)) {
...
void Device::initialize_and_launch_firmware() {
    ...
    const metal_SocDescriptor& soc_d = tt::tt_metal::MetalContext::get_cluster().get_soc_desc(this->id());
    uint64_t pcie_chan_base_addr = tt::tt_metal::MetalContext::get_cluster().get_pcie_base_addr_from_device(this->id());
    uint32_t num_host_channels = tt::tt_metal::MetalContext::get_cluster().get_num_host_channels(this->id());
    uint64_t pcie_chan_end_addr = pcie_chan_base_addr;
    for (int pcie_chan = 0; pcie_chan < num_host_channels; pcie_chan++) {
        pcie_chan_end_addr += tt::tt_metal::MetalContext::get_cluster().get_host_channel_size(this->id(), pcie_chan);
    }
    ...
    uint32_t harvested_noc_rows = CoordinateManager::shuffle_tensix_harvesting_mask_to_noc0_coords(
        tt::tt_metal::MetalContext::get_cluster().get_soc_desc(this->id()).arch,
        tt::tt_metal::MetalContext::get_cluster().get_harvesting_mask(this->id()));
    ...
    tt::tt_metal::MetalContext::get_cluster().l1_barrier(this->id());
    ...
    for (const auto& worker_core : not_done_cores) {
        ...
        tt::tt_metal::MetalContext::get_cluster().deassert_risc_reset_at_core(
...
void Device::compile_command_queue_programs() {
    ...
    if (this->is_mmio_capable()) {
        ...
        if (tt::tt_metal::MetalContext::get_cluster().get_mmio_device_tunnel_count(this->id_) > 0) {
            tunnels_from_mmio_ = tt::tt_metal::MetalContext::get_cluster().get_tunnels_from_mmio_device(this->id_);
            for (auto& tunnel : tunnels_from_mmio_) {
                for (uint32_t tunnel_stop = 0; tunnel_stop < tunnel.size() - 1; tunnel_stop++) {
                    ...
                    uint16_t channel =
                        tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(ds_device_id);
                    // Only one tunneler per connection, use CQ ID 0
                    MetalContext::get_dispatch_core_manager().tunneler_core(device_id, ds_device_id, channel, 0);
                }
            }
        }
    } else {
...
void Device::configure_command_queue_programs() {
    ...
    chip_id_t mmio_device_id = tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(device_id);
    ...
    if (this->is_mmio_capable()) {
        for (chip_id_t serviced_device_id :
             tt::tt_metal::MetalContext::get_cluster().get_devices_controlled_by_mmio_device(device_id)) {
            uint16_t channel =
                tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(serviced_device_id);
            CoreType dispatch_core_type = MetalContext::get_dispatch_core_manager().get_dispatch_core_type();
            ...
            for (uint8_t cq_id = 0; cq_id < num_hw_cqs; cq_id++) {
                ...
                tt::tt_metal::MetalContext::get_cluster().write_sysmem(
                    pointers.data(),
...
bool Device::close() {
    ...
    if (this->id_ != mmio_device_id) {
        for (auto it = not_done_dispatch_cores[mmio_device_id].begin(); it != not_done_dispatch_cores[mmio_device_id].end(); it++) {
            const auto &virtual_core = *it;
            if (tt::tt_metal::MetalContext::get_cluster().is_ethernet_core(virtual_core, this->id_)) {
    ...
    tt::tt_metal::MetalContext::get_cluster().l1_barrier(id_);
...
tt::ARCH Device::arch() const { return tt::tt_metal::MetalContext::get_cluster().arch(); }

int Device::num_dram_channels() const {
    return tt::tt_metal::MetalContext::get_cluster().get_soc_desc(id_).get_num_dram_views();
}

uint32_t Device::l1_size_per_core() const {
    return tt::tt_metal::MetalContext::get_cluster().get_soc_desc(id_).worker_l1_size;
}
uint32_t Device::dram_size_per_channel() const {
    return tt::tt_metal::MetalContext::get_cluster().get_soc_desc(id_).dram_view_size;
}

CoreCoord Device::grid_size() const { return tt::tt_metal::MetalContext::get_cluster().get_soc_desc(id_).grid_size; }

CoreCoord Device::logical_grid_size() const {
    return tt::tt_metal::MetalContext::get_cluster().get_soc_desc(id_).get_grid_size(CoreType::TENSIX);
}

CoreCoord Device::dram_grid_size() const {
    return tt::tt_metal::MetalContext::get_cluster().get_soc_desc(id_).get_dram_grid_size();
}
...
CoreCoord Device::virtual_core_from_logical_core(const CoreCoord &logical_coord, const CoreType& core_type) const {
    return tt::tt_metal::MetalContext::get_cluster().get_virtual_coordinate_from_logical_coordinates(
        this->id_, logical_coord, core_type);
}

CoreCoord Device::virtual_core_from_physical_core(const CoreCoord& physical_coord) const {
    return tt::tt_metal::MetalContext::get_cluster().get_virtual_coordinate_from_physical_coordinates(
...
CoreCoord Device::logical_core_from_ethernet_core(const CoreCoord &ethernet_core) const {
    return tt::tt_metal::MetalContext::get_cluster().get_logical_ethernet_core_from_virtual(this->id(), ethernet_core);
}
...
CoreCoord Device::dram_core_from_dram_channel(uint32_t dram_channel) const {
    return tt::tt_metal::MetalContext::get_cluster().get_soc_desc(id_).get_preferred_worker_core_for_dram_view(
        dram_channel);
}

CoreCoord Device::logical_core_from_dram_channel(uint32_t dram_channel) const {
    const metal_SocDescriptor& soc_desc = tt::tt_metal::MetalContext::get_cluster().get_soc_desc(this->id_);
    return tt::tt_metal::MetalContext::get_cluster().get_soc_desc(id_).get_logical_core_for_dram_view(dram_channel);
}

uint32_t Device::dram_channel_from_logical_core(const CoreCoord& logical_core) const {
    const metal_SocDescriptor& soc_desc = tt::tt_metal::MetalContext::get_cluster().get_soc_desc(this->id_);
    return tt::tt_metal::MetalContext::get_cluster().get_soc_desc(id_).get_dram_channel_from_logical_core(logical_core);
}
