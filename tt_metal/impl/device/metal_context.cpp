...
Cluster& MetalContext::get_cluster() { return MetalContext::instance().cluster_; }

dispatch_core_manager& MetalContext::get_dispatch_core_manager() {
    TT_FATAL(
        MetalContext::instance().dispatch_core_manager_, "Trying to get dispatch_core_manager before intializing it.");
    return *MetalContext::instance().dispatch_core_manager_;
}

DispatchQueryManager& MetalContext::get_dispatch_query_manager() {
    TT_FATAL(
        MetalContext::instance().dispatch_query_manager_,
        "Trying to get dispatch_query_manager before intializing it.");
    return *MetalContext::instance().dispatch_query_manager_;
}
