...
void DevicePool::initialize_active_devices() const {
    ...
    auto iterate_required_devices = [&](IDevice* mmio_device, const std::function<void(IDevice * device)>&& func) {
        auto tunnels_from_mmio = tt::Cluster::instance().get_tunnels_from_mmio_device(mmio_device->id());
    ...
    for (auto device : active_devices) {
        // For Galaxy init, we only need to loop over mmio devices
        const auto& mmio_device_id = tt::Cluster::instance().get_associated_mmio_device(device->id());
