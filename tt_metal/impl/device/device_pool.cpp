...
std::pair<int, int> get_cpu_cores_for_dispatch_threads(
    ...
    int numa_node_for_device =
        tt::tt_metal::MetalContext::get_cluster().get_numa_node_for_device(mmio_controlled_device_id);
...
void DevicePool::init_profiler_devices() const {
    ...
    for (const auto& dev : this->get_all_active_devices()) {
        ...
        const auto& mmio_device_id = tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(dev->id());
        if (mmio_device_id != dev->id()) {
            continue;
        }
        auto tunnels_from_mmio = tt::tt_metal::MetalContext::get_cluster().get_tunnels_from_mmio_device(mmio_device_id);
...
void DevicePool::initialize(
    ...
    bool skip = not tt::tt_metal::MetalContext::get_cluster().is_galaxy_cluster();
    std::vector<chip_id_t> target_mmio_ids;
    for (const auto& device_id : device_ids) {
        TT_FATAL(
            device_id < tt::tt_metal::MetalContext::get_cluster().number_of_devices(),
            "Device index {} out of range. There are {} devices available.",
            device_id,
            tt::tt_metal::MetalContext::get_cluster().number_of_devices());
        const auto& mmio_device_id = tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(device_id);
    ...
    if (target_mmio_ids.size() != tt::tt_metal::MetalContext::get_cluster().number_of_pci_devices()) {
        log_warning(
            tt::LogMetal,
    ...
    tt::tt_metal::MetalContext::get_cluster().set_internal_routing_info_for_ethernet_cores(true, target_mmio_ids);
...
void DevicePool::initialize_active_devices() const {
    ...
    FabricConfig fabric_config = tt::tt_metal::MetalContext::get_cluster().get_fabric_config();
    if (fabric_config == FabricConfig::FABRIC_1D || fabric_config == FabricConfig::FABRIC_2D ||
        ...
        if (fabric_config == FabricConfig::FABRIC_2D || fabric_config == FabricConfig::FABRIC_2D_PUSH) {
            ...
            tt::tt_metal::MetalContext::get_cluster().get_control_plane()->write_routing_tables_to_all_chips();
        }
...
void DevicePool::activate_device(chip_id_t id) {
    TT_FATAL(
        id < tt::tt_metal::MetalContext::get_cluster().number_of_devices(),
        "Device index {} out of range. There are {} devices available.",
        id,
        tt::tt_metal::MetalContext::get_cluster().number_of_devices());
...
void DevicePool::add_devices_to_pool(const std::vector<chip_id_t>& device_ids) {
    ...
    if (this->skip_remote_devices) {
        for (const auto& device_id : device_ids) {
            const auto& mmio_device_id =
                tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(device_id);
        ...
        for (const auto& device_id : device_ids) {
            ...
            for (const auto& mmio_controlled_device_id :
                 tt::tt_metal::MetalContext::get_cluster().get_devices_controlled_by_mmio_device(mmio_device_id)) {
    ...
    if (fabric_config == FabricConfig::FABRIC_1D || fabric_config == FabricConfig::FABRIC_2D ||
        fabric_config == FabricConfig::FABRIC_2D_PUSH) {
        for (int i = 0; i < tt::tt_metal::MetalContext::get_cluster().number_of_devices(); i++) {
    ...
    if (fabric_config == FabricConfig::FABRIC_1D) {
        ...
        auto wait_for_handshake = [&](IDevice* dev) {
            ...
            auto fabric_ethernet_channels =
                tt::tt_metal::MetalContext::get_cluster().get_fabric_ethernet_channels(dev->id());
            if (fabric_ethernet_channels.empty()) {
                return;
            }

            tt_fabric::chan_id_t fabric_master_router_chan = *(fabric_ethernet_channels.begin());
            CoreCoord virtual_eth_core = tt::tt_metal::MetalContext::get_cluster().get_virtual_eth_core_from_channel(
        ...
        for (const auto& dev : this->get_all_active_devices()) {
            if (tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(dev->id()) != dev->id()) {
                continue;
            }

            auto tunnels_from_mmio = tt::tt_metal::MetalContext::get_cluster().get_tunnels_from_mmio_device(dev->id());
            ...
            if (tt::tt_metal::MetalContext::get_cluster().get_cluster_type() != tt::ClusterType::TG) {
        ...
        for (const auto& dev : this->get_all_active_devices()) {
            ...
            auto [mesh_id, chip_id] =
                tt::tt_metal::MetalContext::get_cluster().get_control_plane()->get_mesh_chip_id_from_physical_chip_id(
                    dev->id());
            auto num_routers =
                tt::tt_metal::MetalContext::get_cluster().get_control_plane()->get_num_active_fabric_routers(
                    mesh_id, chip_id);
...
void DevicePool::init_firmware_on_active_devices() const {
    ...
    for (const auto& dev : active_devices) {
        ...
        log_debug(
            ...
            mmio_device_id,
            tt::tt_metal::MetalContext::get_cluster().get_mmio_device_tunnel_count(mmio_device_id));
        log_debug(
            tt::LogMetal,
            "MMIO Device {} Tunnel Depth: {}",
            mmio_device_id,
            tt::tt_metal::MetalContext::get_cluster().get_mmio_device_max_tunnel_depth(mmio_device_id));
        log_debug(
            tt::LogMetal,
            "MMIO Device {} Tunnel Stop: {}",
            mmio_device_id,
            tt::tt_metal::MetalContext::get_cluster().get_device_tunnel_depth(mmio_device_id));
...
void DevicePool::close_devices(const std::vector<IDevice*>& devices) {
    ...
    for (const auto& dev_id : devices_to_close) {
        ...
        if (tt::tt_metal::MetalContext::get_cluster().is_galaxy_cluster() and dev->is_mmio_capable()) {
            continue;
        }
    ...
    if (fabric_config == FabricConfig::FABRIC_1D) {
        ...
        for (const auto& dev : this->get_all_active_devices()) {
            if (dev->is_mmio_capable() &&
                (tt::tt_metal::MetalContext::get_cluster().get_cluster_type() == tt::ClusterType::TG)) {
                // 1d fabric is not launched on TG gateways
                continue;
            }
