...
void insert_stall_cmds(ProgramCommandSequence& program_command_sequence, SubDeviceId sub_device_id, IDevice* device) {
    ...
    auto dispatch_core_config = MetalContext::get_dispatch_core_manager().get_dispatch_core_config();
...
void assemble_device_commands(
    ...
    if (tt_metal::MetalContext::get_dispatch_query_manager().dispatch_s_enabled()) {
...
void reset_worker_dispatch_state_on_device(
    ...
    uint32_t cmd_sequence_sizeB =
        reset_launch_msg_state * MetalContext::get_dispatch_query_manager().dispatch_s_enabled() *
        ...
        (hal_ref.get_alignment(
         ...
         MetalContext::get_dispatch_query_manager().distributed_dispatcher() *
    ...
    if (reset_launch_msg_state) {
        if (MetalContext::get_dispatch_query_manager().dispatch_s_enabled()) {
            uint16_t index_bitmask = 0;
    ...
    for (uint32_t i = 0; i < num_sub_devices; ++i) {
        ...
        if (MetalContext::get_dispatch_query_manager().distributed_dispatcher()) {
...
void set_go_signal_noc_data_on_dispatch(
    ...
    void* cmd_region = manager.issue_queue_reserve(cmd_sequence_sizeB, cq_id);
    HugepageDeviceCommand command_sequence(cmd_region, cmd_sequence_sizeB);
    DispatcherSelect dispatcher_for_go_signal = MetalContext::get_dispatch_query_manager().dispatch_s_enabled()
