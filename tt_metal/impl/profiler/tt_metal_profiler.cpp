...
#if defined(TRACY_ENABLE)
    const metal_SocDescriptor& soc_d = tt::tt_metal::MetalContext::get_cluster().get_soc_desc(device_id);

    control_buffer[kernel_profiler::CORE_COUNT_PER_DRAM] = soc_d.profiler_ceiled_core_count_perf_dram_bank;

    for (auto core : tt::tt_metal::MetalContext::get_cluster().get_virtual_routing_to_profiler_flat_id(device_id)) {
        HalProgrammableCoreType CoreType;
        auto curr_core = core.first;
        if (tt::tt_metal::MetalContext::get_cluster().is_worker_core(curr_core, device_id)) {
            CoreType = HalProgrammableCoreType::TENSIX;
        } else {
            CoreType = tt_metal::HalProgrammableCoreType::ACTIVE_ETH;
            auto active_eth_cores = tt::tt_metal::MetalContext::get_cluster().get_active_ethernet_cores(device_id);
            if (active_eth_cores.find(tt::tt_metal::MetalContext::get_cluster().get_logical_ethernet_core_from_virtual(
                    device_id, curr_core)) != active_eth_cores.end()) {
                CoreType = tt_metal::HalProgrammableCoreType::ACTIVE_ETH;
            }
            auto idle_eth_cores = tt::tt_metal::MetalContext::get_cluster().get_inactive_ethernet_cores(device_id);
            if (idle_eth_cores.find(tt::tt_metal::MetalContext::get_cluster().get_logical_ethernet_core_from_virtual(
...
void syncDeviceHost(IDevice* device, CoreCoord logical_core, bool doHeader) {
    ...
    for (int i = 0; i < sampleCount; i++) {
        ...
        tt::tt_metal::MetalContext::get_cluster().write_reg(&sinceStart, tt_cxy_pair(device_id, core), control_addr);
...
void syncDeviceDevice(chip_id_t device_id_sender, chip_id_t device_id_receiver) {
    ...
    if (device_sender != nullptr and device_receiver != nullptr) {
        ...
        while ((device_id_receiver != device_id_receiver_curr) and (eth_sender_core_iter != active_eth_cores.end())) {
            ...
            if (not tt::tt_metal::MetalContext::get_cluster().is_ethernet_link_up(
...
void ProfilerSync(ProfilerSyncState state) {
    ...
    if (state == ProfilerSyncState::INIT) {
        ...
        auto ethernet_connections = tt::tt_metal::MetalContext::get_cluster().get_ethernet_connections();
...
void InitDeviceProfiler(IDevice* device) {
    ...
    if (getDeviceProfilerState()) {
        ...
        uint32_t dramBankCount = tt::tt_metal::MetalContext::get_cluster().get_soc_desc(device_id).get_num_dram_views();
        uint32_t coreCountPerDram =
            tt::tt_metal::MetalContext::get_cluster().get_soc_desc(device_id).profiler_ceiled_core_count_perf_dram_bank;
                    ...
                    if (tt::tt_metal::MetalContext::get_cluster().is_worker_core(curr_core, device_id)) {
                        ...
                        bool is_active_eth_core =
                            active_eth_cores.find(
                                tt::tt_metal::MetalContext::get_cluster().get_logical_ethernet_core_from_virtual(
                                    device_id, curr_core)) != active_eth_cores.end();
