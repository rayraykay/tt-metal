...
tt::tt_metal::DispatchCoreConfig dispatch_core_config() {
    return tt::tt_metal::MetalContext::get_dispatch_core_manager().get_dispatch_core_config();
}
...
tt_cxy_pair dispatch_core(uint8_t cq_id) {
    ...
    for (chip_id_t device_id = 0; device_id < tt::tt_metal::MetalContext::get_cluster().number_of_devices();
         device_id++) {
        uint16_t channel = tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(device_id);
        if (tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(device_id) == device_id) {
            ...
            // skipped.
            if (not tt::tt_metal::MetalContext::get_dispatch_core_manager().is_dispatcher_core_allocated(
                    device_id, channel, cq_id) or
                tt::tt_metal::MetalContext::get_cluster().is_galaxy_cluster()) {
                continue;
            }
            dispatch_core =
                tt::tt_metal::MetalContext::get_dispatch_core_manager().dispatcher_core(device_id, channel, cq_id);
        } else {
            // Dispatch core is not allocated on this Non-MMIO device, skip it
            if (not tt::tt_metal::MetalContext::get_dispatch_core_manager().is_dispatcher_d_core_allocated(
                    device_id, channel, cq_id)) {
                continue;
            }
            dispatch_core =
                tt::tt_metal::MetalContext::get_dispatch_core_manager().dispatcher_d_core(device_id, channel, cq_id);
        }
...
std::vector<CoreCoord> get_consistent_logical_cores(
    ...
    auto user_chips = tt::tt_metal::MetalContext::get_cluster().user_exposed_chip_ids();
