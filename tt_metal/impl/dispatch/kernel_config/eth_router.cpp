...
void EthRouterKernel::GenerateStaticConfigs() {
    ...
    if (as_mux_) {
        uint16_t channel = tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(
            servicing_device_id_);  // TODO: can be mmio
        logical_core_ =
            MetalContext::get_dispatch_core_manager().mux_core(servicing_device_id_, channel, placement_cq_id_);
        ...
        logical_core_ =
            MetalContext::get_dispatch_core_manager().demux_d_core(device_->id(), channel, placement_cq_id_);
        static_config_.rx_queue_start_addr_words = my_dispatch_constants.dispatch_buffer_base() >> 4;
