...
void EthTunnelerKernel::GenerateStaticConfigs() {
    ...
    if (this->IsRemote()) {
        uint16_t channel =
            tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(downstream_device_id);
        logical_core_ = MetalContext::get_dispatch_core_manager().tunneler_core(
        ...
        logical_core_ =
            MetalContext::get_dispatch_core_manager().us_tunneler_core_local(device_->id(), channel, cq_id_);
    }
...
void EthTunnelerKernel::GenerateDependentConfigs() {
    if (this->IsRemote()) {
        ...
        uint16_t downstream_channel =
            tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(downstream_device_id);
        tt_cxy_pair paired_logical_core = MetalContext::get_dispatch_core_manager().us_tunneler_core_local(
        ...
        uint16_t channel = tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(device_id_);
        tt_cxy_pair paired_logical_core =
            MetalContext::get_dispatch_core_manager().tunneler_core(upstream_device_id, device_id_, channel, cq_id_);
        tt_cxy_pair paired_physical_coord =
            tt::tt_metal::MetalContext::get_cluster().get_virtual_coordinate_from_logical_coordinates(
                paired_logical_core, CoreType::ETH);
