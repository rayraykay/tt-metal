...
SystemMemoryManager::SystemMemoryManager(chip_id_t device_id, uint8_t num_hw_cqs) :
    ...
    fast_write_callable(tt::tt_metal::MetalContext::get_cluster().get_fast_pcie_static_tlb_write_callable(device_id)),
    bypass_enable(false),
    ...
    uint16_t channel = tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(device_id);
    char* hugepage_start =
        (char*)tt::tt_metal::MetalContext::get_cluster().host_dma_address(0, mmio_device_id, channel);
    ...
    if (cq_size_override_env != nullptr) {
        ...
        this->cq_size =
            tt::tt_metal::MetalContext::get_cluster().get_host_channel_size(mmio_device_id, channel) / num_hw_cqs;
    ...
    CoreType core_type = tt::tt_metal::MetalContext::get_dispatch_core_manager().get_dispatch_core_type();
    ...
    for (uint8_t cq_id = 0; cq_id < num_hw_cqs; cq_id++) {
        tt_cxy_pair prefetcher_core =
            tt::tt_metal::MetalContext::get_dispatch_core_manager().prefetcher_core(device_id, channel, cq_id);
        ...
        this->prefetch_q_writers.emplace_back(
            tt::tt_metal::MetalContext::get_cluster().get_static_tlb_writer(this->prefetcher_cores[cq_id]));

        tt_cxy_pair completion_queue_writer_core =
            tt::tt_metal::MetalContext::get_dispatch_core_manager().completion_queue_writer_core(
        ...
        const std::tuple<uint32_t, uint32_t> completion_interface_tlb_data = tt::tt_metal::MetalContext::get_cluster()
...
void SystemMemoryManager::issue_queue_push_back(uint32_t push_size_B, const uint8_t cq_id) {
    ...
    chip_id_t mmio_device_id = tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(this->device_id);
    uint16_t channel = tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(this->device_id);
...
void SystemMemoryManager::fetch_queue_reserve_back(const uint8_t cq_id) {
    ...
    auto wait_for_fetch_q_space = [&]() {
        ...
        while (this->prefetch_q_dev_ptrs[cq_id] == this->prefetch_q_dev_fences[cq_id]) {
            tt::tt_metal::MetalContext::get_cluster().read_core(
...
void SystemMemoryManager::fetch_queue_write(uint32_t command_size_B, const uint8_t cq_id, bool stall_prefetcher) {
    CoreType dispatch_core_type = tt::tt_metal::MetalContext::get_dispatch_core_manager().get_dispatch_core_type();
