...
const tt_cxy_pair& dispatch_core_manager::tunneler_core(
    ...
    auto [us_core, ds_core] = tt::tt_metal::MetalContext::get_cluster().get_eth_tunnel_core(
    ...
    for (chip_id_t device_id = 0; device_id < tt::tt_metal::MetalContext::get_cluster().number_of_devices();
        ...
        if (dispatch_core_config.get_core_type() == CoreType::ETH) {
            for (const auto& idle_eth_core :
                 tt::tt_metal::MetalContext::get_cluster().get_inactive_ethernet_cores(device_id)) {
...
void dispatch_core_manager::log_dispatch_assignment(
    ...
    log_debug(
        ...
        name,
        cxy.str(),
        tt::tt_metal::MetalContext::get_cluster()
