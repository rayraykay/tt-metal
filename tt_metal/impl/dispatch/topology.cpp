...
std::vector<DispatchKernelNode> generate_nodes(const std::set<chip_id_t>& device_ids, uint32_t num_hw_cqs) {
    ...
    uint32_t total_devices = tt::tt_metal::MetalContext::get_cluster().number_of_devices();
    TT_ASSERT(
    ...
    for (auto id : device_ids) {
        if (tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(id) == id) {
        ...
        if (tt::tt_metal::MetalContext::get_cluster().is_galaxy_cluster()) {
            ...
            for (auto mmio_device_id : mmio_devices) {
                ...
                for (const auto& tunnel :
                     tt::tt_metal::MetalContext::get_cluster().get_tunnels_from_mmio_device(mmio_device_id)) {
            ...
            for (auto mmio_device_id : mmio_devices) {
                ...
                for (auto id : remote_devices) {
                    if (tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(id) == mmio_device_id) {
                        remote_device_id = id;
                        found_remote = true;
                        break;
                    }
...
void populate_fd_kernels(const std::vector<DispatchKernelNode>& nodes) {
    ...
    for (const auto& node : nodes) {
        ...
        if (tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(node.device_id) == node.device_id) {
    ...
    for (auto mmio_device_id : mmio_device_ids) {
        if (tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(mmio_device_id) != mmio_device_id) {
            continue;
        }
        ...
        for (auto tunnel : tt::tt_metal::MetalContext::get_cluster().get_tunnels_from_mmio_device(mmio_device_id)) {
    ...
    for (auto fd_kernel : node_id_to_kernel) {
        chip_id_t mmio_device_id =
            tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(fd_kernel->GetDeviceId());
    ...
    for (auto fd_kernel : node_id_to_kernel) {
        if (auto router_kernel = dynamic_cast<EthRouterKernel*>(fd_kernel)) {
            ...
            chip_id_t mmio_device_id =
                tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(router_device_id);
    ...
    for (auto fd_kernel : node_id_to_kernel) {
        ...
        if (auto tunneler_kernel = dynamic_cast<EthTunnelerKernel*>(fd_kernel)) {
            ...
            chip_id_t mmio_device_id =
                tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(router_kernel->GetDeviceId());
            if (router_kernel->GetDeviceId() == mmio_device_id) {
                uint32_t num_tunnels =
                    tt::tt_metal::MetalContext::get_cluster().get_tunnels_from_mmio_device(mmio_device_id).size();
...
void configure_dispatch_cores(IDevice* device) {
    ...
    if (device->is_mmio_capable()) {
        for (chip_id_t serviced_device_id :
             tt::tt_metal::MetalContext::get_cluster().get_devices_controlled_by_mmio_device(device->id())) {
            uint16_t channel =
                tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(serviced_device_id);
            for (uint8_t cq_id = 0; cq_id < device->num_hw_cqs(); cq_id++) {
                tt_cxy_pair completion_q_writer_location =
                    MetalContext::get_dispatch_core_manager().completion_queue_writer_core(
...
std::unique_ptr<Program> create_and_compile_2d_fabric_program(IDevice* device, FabricConfig fabric_config) {
    ...
    auto router_chans = tt::tt_metal::MetalContext::get_cluster().get_fabric_ethernet_channels(device->id());
    ...
    for (const auto& router_chan : router_chans) {
        CoreCoord virtual_eth_core =
            tt::tt_metal::MetalContext::get_cluster().get_virtual_eth_core_from_channel(device->id(), router_chan);
...
std::unique_ptr<Program> create_and_compile_1d_fabric_program(IDevice* device, bool wrap_around_mesh = true) {
    ...
    for (const auto& [direction, remote_chip_id] : chip_neighbors) {
        for (const auto& eth_chan : active_fabric_eth_channels[direction]) {
            auto eth_logical_core = tt::tt_metal::MetalContext::get_cluster()
    ...
    uint32_t master_edm_chan =
        *(tt::tt_metal::MetalContext::get_cluster().get_fabric_ethernet_channels(device->id()).begin());
...
std::unique_ptr<Program> create_and_compile_fabric_program(IDevice* device) {
    auto fabric_config = tt::tt_metal::MetalContext::get_cluster().get_fabric_config();
