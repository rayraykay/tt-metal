...
HWCommandQueue::HWCommandQueue(
    ...
    chip_id_t mmio_device_id = tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(device_->id());
    uint16_t channel = tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(device_->id());
    this->size_B_ = tt::tt_metal::MetalContext::get_cluster().get_host_channel_size(mmio_device_id, channel) /
                    device_->num_hw_cqs();
    ...
    CoreType core_type = MetalContext::get_dispatch_core_manager().get_dispatch_core_type();
    if (this->device_->num_hw_cqs() == 1 or core_type == CoreType::WORKER) {
        ...
        enqueue_program_dispatch_core =
            MetalContext::get_dispatch_core_manager().dispatcher_s_core(device_->id(), channel, id);
    } else {
        if (device_->is_mmio_capable()) {
            enqueue_program_dispatch_core =
                MetalContext::get_dispatch_core_manager().dispatcher_core(device_->id(), channel, id);
        } else {
            enqueue_program_dispatch_core =
                MetalContext::get_dispatch_core_manager().dispatcher_d_core(device_->id(), channel, id);
        }
    }
    ...
    tt_cxy_pair completion_q_writer_location =
        MetalContext::get_dispatch_core_manager().completion_queue_writer_core(device_->id(), channel, this->id_);
    ...
    if (is_sharded(buffer_obj.buffer_layout())) {
        ...
        for (uint32_t core_id = 0; core_id < buffer_obj.num_cores(); ++core_id) {
            buffer_dispatch::copy_sharded_buffer_from_core_to_completion_queue(
                ...
                sub_device_ids,
                cores[core_id],
                MetalContext::get_dispatch_core_manager().get_dispatch_core_type());
...
void HWCommandQueue::enqueue_write_buffer(
    ...
    sub_device_ids = buffer_dispatch::select_sub_device_ids(this->device_, sub_device_ids);
    auto dispatch_core_type = MetalContext::get_dispatch_core_manager().get_dispatch_core_type();
...
CoreType HWCommandQueue::get_dispatch_core_type() {
    return MetalContext::get_dispatch_core_manager().get_dispatch_core_type();
}
...
void HWCommandQueue::read_completion_queue() {
    chip_id_t mmio_device_id =
        tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(this->device_->id());
    uint16_t channel = tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(this->device_->id());
    while (true) {
