...
uint32_t compute_build_key(chip_id_t device_id, uint8_t num_hw_cqs) {
    ...
    if (not hal_ref.is_coordinate_virtualization_enabled()) {
        ...
        build_key |= tt::tt_metal::MetalContext::get_cluster().get_harvesting_mask(device_id);
    } else {
        ...
        build_key |=
            (std::bitset<harvesting_map_bits>(tt::tt_metal::MetalContext::get_cluster().get_harvesting_mask(device_id))
                 .count());
    }
    return build_key;
}
...
void BuildEnvManager::add_build_env(chip_id_t device_id, uint8_t num_hw_cqs) {
    ...
    device_id_to_build_env_[device_id].build_env.init(
        build_key, tt::tt_metal::MetalContext::get_cluster().arch(), device_kernel_defines);
