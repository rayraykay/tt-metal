...
private:
    std::pair<std::optional<CoreCoord>, std::shared_ptr<tt_metal::Buffer>> assign_tensix_and_allocate_buffer(
        ...
        const auto& soc_d = tt::tt_metal::MetalContext::get_cluster().get_soc_desc(logical_eth_core.chip);
    ...
    void initialize_sender_receiver_pairs(const TestParams& params) {
        ...
        auto all_eth_connections = tt::tt_metal::MetalContext::get_cluster().get_ethernet_connections();
        ...
        while (!chip_q.empty()) {
            ...
            for (chip_id_t connected_chip : tt::tt_metal::MetalContext::get_cluster().get_ethernet_connected_device_ids(chip_id)) {
        ...
        for (auto sender_chip_id : sender_chips) {
            auto non_tunneling_eth_cores =
                tt::tt_metal::MetalContext::get_cluster().get_active_ethernet_cores(sender_chip_id, !slow_dispath_mode);
            for (auto logical_active_eth : non_tunneling_eth_cores) {
                auto sender_eth = tt_cxy_pair(sender_chip_id, logical_active_eth);
                auto receiver_eth_tuple = tt::tt_metal::MetalContext::get_cluster().get_connected_ethernet_core(
