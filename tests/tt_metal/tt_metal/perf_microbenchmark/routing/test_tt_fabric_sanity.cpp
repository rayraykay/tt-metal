...
typedef struct test_board {
    ...
    test_board(std::string& board_type_) {
        ...
        if (metal_fabric_init_level == 0) {
            control_plane = tt::tt_metal::MetalContext::get_cluster().get_control_plane();
    ...
    void generate_tx_rx_map(
        ...
        for (auto chip_id : physical_chip_ids) {
            auto neighbors =
                tt::tt_metal::MetalContext::get_cluster().get_ethernet_cores_grouped_by_connected_chips(chip_id);
...
typedef struct test_device {
    ...
    test_device(chip_id_t chip_id_, test_board_t* board_handle_) {
        ...
        soc_desc = tt::tt_metal::MetalContext::get_cluster().get_soc_desc(physical_chip_id);
