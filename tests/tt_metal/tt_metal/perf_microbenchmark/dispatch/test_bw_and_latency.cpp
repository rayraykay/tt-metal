...
int main(int argc, char** argv) {
    ...
    try {
        ...
        void* host_pcie_base =
            (void*)tt::tt_metal::MetalContext::get_cluster().host_dma_address(0, mmio_device_id, channel);
        uint64_t dev_pcie_base = tt::tt_metal::MetalContext::get_cluster().get_pcie_base_addr_from_device(device->id());
        uint64_t pcie_offset = 1024 * 1024 * 50;  // beyond where FD will write...maybe

        const metal_SocDescriptor& soc_d = tt::tt_metal::MetalContext::get_cluster().get_soc_desc(device->id());
        switch (source_mem_g) {
            case 0:
            default: {
                src_mem = "FROM_PCIE";
            ...
            if (hammer_write_reg_g || hammer_pcie_g) {
                ...
                while (!done) {
                    if (hammer_write_reg_g) {
                        tt::tt_metal::MetalContext::get_cluster().write_reg(
