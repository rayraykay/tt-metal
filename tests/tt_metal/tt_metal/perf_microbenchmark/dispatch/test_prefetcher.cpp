...
void init(int argc, char** argv) {
    auto default_settings =
        DispatchSettings::defaults(DISPATCH_CORE_TYPE, tt::tt_metal::MetalContext::get_cluster(), 1);
...
void configure_for_single_chip(
    ...
    chip_id_t mmio_device_id = tt::tt_metal::MetalContext::get_cluster().get_associated_mmio_device(device->id());
    uint16_t channel = tt::tt_metal::MetalContext::get_cluster().get_assigned_channel_for_device(device->id());
    host_hugepage_base = (void*)tt::tt_metal::MetalContext::get_cluster().host_dma_address(0, mmio_device_id, channel);
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (test_device_id_g == 0) {
            ...
            auto remote_chips =
                tt::tt_metal::MetalContext::get_cluster().get_devices_controlled_by_mmio_device(device_id_l);
        ...
        tt::Writer prefetch_q_writer = tt::tt_metal::MetalContext::get_cluster().get_static_tlb_writer(
        ...
        tt::tt_metal::MetalContext::get_cluster().dram_barrier(device->id());
        if (test_device_id_g != 0) {
            tt::tt_metal::MetalContext::get_cluster().l1_barrier(device_r->id());
            tt::tt_metal::MetalContext::get_cluster().dram_barrier(device_r->id());
        }
