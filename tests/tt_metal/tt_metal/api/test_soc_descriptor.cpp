...
std::unordered_set<int> get_harvested_rows(chip_id_t device_id) {
    uint32_t harvested_rows_mask = CoordinateManager::shuffle_tensix_harvesting_mask_to_noc0_coords(
        tt::tt_metal::MetalContext::get_cluster().get_soc_desc(device_id).arch,
        tt::tt_metal::MetalContext::get_cluster().get_harvesting_mask(device_id));
...
TEST(SOC, TensixValidateLogicalToPhysicalCoreCoordHostMapping) {
    ...
    for (int device_id = 0; device_id < num_devices; device_id++) {
        ...
        uint32_t harvested_rows_mask = tt::tt_metal::MetalContext::get_cluster().get_harvesting_mask(device_id);
