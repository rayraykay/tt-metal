...
bool is_galaxy_device(const chip_id_t device_id) {
    return tt::tt_metal::MetalContext::get_cluster().get_board_type(device_id) == BoardType::GALAXY;
}

bool is_n150_device(const chip_id_t device_id) {
    return tt::tt_metal::MetalContext::get_cluster().get_board_type(device_id) == BoardType::N150;
}
...
std::unordered_set<chip_id_t> get_ethernet_connected_device_ids(const chip_id_t device_id) {
    ...
    for (const CoreCoord& ethernet_core : active_ethernet_cores) {
        const auto& [connected_device_id, _] =
            tt::tt_metal::MetalContext::get_cluster().get_connected_ethernet_core({device_id, ethernet_core});
...
TEST_F(GalaxyFixture, ValidateAllGalaxyChipsAreUnharvested) {
    for (IDevice* device : this->devices_) {
        const chip_id_t device_id = device->id();
        if (is_galaxy_device(device_id)) {
            const uint32_t harvest_mask = tt::tt_metal::MetalContext::get_cluster().get_harvesting_mask(device_id);
...
TEST_F(GalaxyFixture, ValidateAllMMIOChipsHaveSingleRowHarvested) {
    for (IDevice* device : this->devices_) {
        ...
        if (!is_galaxy_device(device_id)) {
            ...
            uint32_t harvest_mask = tt::tt_metal::MetalContext::get_cluster().get_harvesting_mask(device_id);
            while (harvest_mask) {
                if (harvest_mask & 1) {
                    num_rows_harvested++;
                }
...
TEST_F(TGFixture, ValidateNumMMIOChips) {
    const size_t num_mmio_chips = tt::tt_metal::MetalContext::get_cluster().number_of_pci_devices();
    ASSERT_TRUE(num_mmio_chips == 4) << "Detected " << num_mmio_chips << " MMIO chips" << std::endl;
}

TEST_F(TGFixture, ValidateNumGalaxyChips) {
    const size_t num_galaxy_chips = tt::tt_metal::MetalContext::get_cluster().number_of_user_devices();
