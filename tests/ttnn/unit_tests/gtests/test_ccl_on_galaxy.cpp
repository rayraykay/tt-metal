...
bool is_tg_system() {
    const bool is_galaxy_system = tt::tt_metal::MetalContext::get_cluster().is_galaxy_cluster();
    const size_t num_mmio_devices = tt::tt_metal::MetalContext::get_cluster().number_of_pci_devices();
    const size_t num_devices = tt::tt_metal::MetalContext::get_cluster().number_of_user_devices();
...
void validate_num_tunnels_and_tunnel_depth() {
    const uint32_t num_devices_in_tunnel = tt::tt_metal::MetalContext::get_cluster().get_mmio_device_max_tunnel_depth(0);
    const uint32_t num_mmio_devices = tt::tt_metal::MetalContext::get_cluster().number_of_pci_devices();
    const uint32_t cluster_tunnel_count = tt::tt_metal::MetalContext::get_cluster().get_mmio_device_tunnel_count(0);
    TT_FATAL(
...
TEST(GalaxyTests, TestAllGatherDeadlock) {
    if (not tt::tt_metal::MetalContext::get_cluster().is_galaxy_cluster()) {
