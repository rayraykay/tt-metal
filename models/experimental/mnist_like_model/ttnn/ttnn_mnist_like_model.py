...
class TtMnistLikeModel:
    def __init__(self, device, parameters: ParameterDict):
        self.device = device
        self.parameters = parameters.conv_args
        self.parameter_pth = parameters
        self.conv1 = MnistLikeModelConv2D(parameters.conv_args.conv1, parameters.conv1, device=device, num_cores_nhw=56)
        self.conv2 = MnistLikeModelConv2D(parameters.conv_args.conv2, parameters.conv2, device=device, num_cores_nhw=48)
        self.conv3 = MnistLikeModelConv2D(parameters.conv_args.conv3, parameters.conv3, device=device, num_cores_nhw=16)
        self.conv4 = MnistLikeModelConv2D(parameters.conv_args.conv4, parameters.conv4, device=device)
    ...
    def __call__(self, input):
        ...
        maxpool4_out = ttnn.reshape(maxpool4_out, (2, -1))
